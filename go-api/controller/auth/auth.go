...
func Register(c *gin.Context) {
	...
	orm.Db.Create(&user)
	if user.ID > 0 {
		c.JSON(200, gin.H{"status": "ok", "msg": "Create new user successfully",
			"userID": user.ID})
	} else {
		c.JSON(200, gin.H{"status": "error", "msg": "Failed to create user",
			"userID": user.ID})
	}
}
...
func Login(c *gin.Context) {
	...
	orm.Db.Where("username = ?", json.Username).First(&userExist)
	if userExist.ID == 0 {
		c.JSON(200, gin.H{"status": "error", "msg": "User not found"})
		return
	}
	...
	if err == nil {
		...
		c.JSON(200, gin.H{"status": "ok", "msg": "Login Successful", "token": tokenString, "user": userExist})
	} else {
		c.JSON(200, gin.H{"status": "error", "msg": "Password incorrect!"})
	}
}
